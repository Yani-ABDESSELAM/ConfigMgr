...
The Policy for the Task Sequence must already be on the target device!
#>

[CmdletBinding()]
Param (
  [Parameter(Mandatory = $true)][string]
  $TSPackageID = "XYZ00000"
)
      ...
      If ([regex]::match($string.Substring(0, $TestLength), $SccmRegexShort).value) { 
        ...
        For ($Counter = 1 ; $Counter -Lt $Bottom + 1; $Counter++) { 
          ...
          Switch ($Hash.Item('Type')) {
            0 { $Hash.Add('TypeName', 'Status') }
            1 { $Hash.Add('TypeName', 'Info') }
            2 { $Hash.Add('TypeName', 'Error') }
            3 { $Hash.Add('TypeName', 'Warning') }
            4 { $Hash.Add('TypeName', 'Verbose') }
            5 { $Hash.Add('TypeName', 'Debug') }
          }
...
#$TSPackageID = "SEA00000"  - Handy for testing.
...
If ($TSName) {
  ...
  $ExecMgrLogString = $ExecMgrLogString | Where-Object -FilterScript { $_.Time -eq ($ExecMgrLogString.time | Measure-Object -Maximum).Maximum }
  ...
  $ExecMgrLogMWString = $ExecMgrLogMWString | Where-Object -FilterScript { $_.Time -eq ($ExecMgrLogMWString.time | Measure-Object -Maximum).Maximum }
...
Else {
  Write-Output "No Task Sequence for Package ID $TSPackageID on this Machine"
}
