...
from CM_XXX.dbo.v_R_System rSys
left join CM_XXX.dbo.vWorkstationStatus HINV
	on rSys.ResourceID = HINV.ResourceID
left join CM_XXX.dbo.vSoftwareInventoryStatus SINV
	on rSys.ResourceID = SINV.ResourceID
left join CM_XXX.dbo.v_UpdateScanStatus UpdateScan
	on rSys.ResourceID = UpdateScan.ResourceID
left join CM_XXX.dbo.v_GS_COMPUTER_SYSTEM CompSys
	on rSys.resourceid = CompSys.ResourceID
left join CM_XXX.dbo.v_GS_OPERATING_SYSTEM OS
	on rSys.ResourceID = OS.ResourceID
left join CM_XXX.dbo.v_GS_PC_BIOS BIOS
	on rSys.ResourceID = BIOS.ResourceID
left join CM_XXX.dbo.v_GS_PHYSICAL_MEMORY Mem
	on rSys.ResourceID = Mem.ResourceID
left join CM_XXX.dbo.v_GS_PROCESSOR Processor
...
left join
	...
	Select ResourceID
		, Size0
		, FreeSpace0
	from CM_XXX.dbo.v_GS_LOGICAL_DISK 
	where DeviceID0 = 'C:'
	) LogicalDisk
	on rSys.ResourceID = LogicalDisk.ResourceID
left join CM_XXX.dbo.v_UserMachineRelationship PrimaryUser
...
left join
	...
	select uss.ResourceID
		...
		,	CM_XXX.dbo.fnConvertBinaryToHexString(convert(VARBINARY(8), isnull(uss.LastErrorCode,0))) as 'HexErrorCode'
	from CM_XXX.dbo.v_UpdateScanStatus uss
	join CM_XXX.dbo.v_R_System rsys 
		on rsys.ResourceID = uss.ResourceID 
			and isnull(rsys.Obsolete0,0)<>1
	join CM_XXX.dbo.v_SoftwareUpdateSource sus 
		on uss.UpdateSource_ID = sus.UpdateSource_ID 
	join CM_XXX.dbo.v_RA_System_SMSAssignedSites sass 
		on uss.ResourceID = sass.ResourceID
	join CM_XXX.dbo.v_StateNames sn 
		on sn.TopicType = 501 
			and sn.StateID = (
...
left JOIN CM_XXX.dbo.v_GS_SYSTEM_CONSOLE_USAGE_MAXGROUP TopUser
	on rSys.ResourceID = TopUser.resourceID
left join
	(
	select ResourceID
		, max(system_ou_name0) as 'OU'
	from CM_XXX.dbo.v_RA_System_SystemOUName
	group by ResourceID
	) OU
