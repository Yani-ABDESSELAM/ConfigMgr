...
ForEach ($Group in $Groups) {
  ...
  Else {
    ...
    New-CMUserCollection -Name $Group -LimitingCollectionName $LimitingCollections -RefreshSchedule $Sched -WhatIf
    Start-Sleep -Seconds 1
    Add-CMDeviceCollectionQueryMembershipRule -CollectionName $Group -QueryExpression $Query -RuleName $Group -WhatIf
  }
}
